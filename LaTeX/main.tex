...
where $\hat{\phi}$ is the known or computed value. This ``physics-informed'' loss function represents the residual of the PDEs and the mismatch at the boundaries, requiring no \emph{a priori} labeled training data. Gradient descent is then used to find optimal weights (and biases). The squared error loss function is minimized when the partial derivatives with respect to 
